  ...
  return (
    <>
      <Section aria-labelledby="fx-title">
        <SectionHeader level={1} label="no origins · build step 4 · fixture" title="Page mode" titleId="fx-title" lead="A sticky level-2 surface as the bar, a reading column at 1120 with 24 / 48 gutters, section rhythm of 96 desktop and 64 mobile, and a footer on the ground." />
      ...
      <Section>
        ...
        <p className="noo-body max-w-[68ch] text-ink-2">
        ...
        <p className="noo-body max-w-[68ch] text-ink-2">A level-3 header is for a sub-section: it follows content, so it takes the 64px above.</p>
        <SectionHeader title="A header with no eyebrow and no lead" />
        <p className="noo-body max-w-[68ch] text-ink-2">Both are optional. The title alone still gets the same spacing.</p>
      </Section>

      <Section>
        <SectionHeader label="scroll test" title="Enough content to scroll under the bar" lead="The bar should stay legible over cards, blobs and text as they pass beneath it." />
          ...
          {(["peach", "lavender", "blue", "yellow", "green", "pink"] as const).map((h, i) => (
            <Card key={h} interactive as="article" className="flex gap-5">
              ...
              <div className="min-w-0">
                ...
                <p className="noo-body-sm mt-1 text-ink-2">Scroll so this card passes under the nav bar. The bar is opaque; the card goes behind it.</p>
