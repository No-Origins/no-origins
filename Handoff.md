...
- **Step 3 — the nine picks, built** (2026-09-14, same branch): Bhargav picked A on all nine boards with one note (no dot on a widget cell's head; a `media` slot for a logo instead). Rules in Admin.md §6.5c. F1–F5 in the package (Scene-Schema.md §10.4; proof re-measured — status and projects identical, work/cases/home < 1 %, interests 4 %, philosophy 6 % from the word's wrap). The editor screen at **`/projects/portfolio/edit`** — `@no-origins/ui/editor` (PropsForm, DocumentForm, Palette, SaveState, CanvasOverlay) + the admin route, `PATCH /api/documents/[id]/draft` with the rev rule — built by an Opus agent in a worktree and merged; verified by signing in and exercising it (select → ring + tag, inspector edit → canvas, autosave saving → saved with rev climbing, drop → node on a box corner, persisted). Preview and Publish are stubs (step 8). A `PropSpec.deprecated` mechanism was added when the first opened draft still carried `dot`. Scripts: `e2e/.mcp/signin.mjs` (mints a magic link with the local service-role key — the local stack allows two sign-in emails an hour), `editor-check.mjs`, `reset-draft.mjs` (gitignored).
...
3. **The editor screen** at `/projects/portfolio/edit` — **boards published 2026-09-14** (canvas *No Origins Editor Screen*, above): S1 selected node · S2 palette landing · S3 autosave states · S4 inspector with none/several, plus F1–F5 from step 2's findings. **Picked and built 2026-09-14** — see above. Next: step 8 (publish and versions — R1's labelled integers, the probes gating Publish, the ISR webhook and the read-back), full views as pages mounted somewhere, and the two open questions in Scene-Schema.md §10.4 (the word's wrap; the sample tag as child or sibling). His rule: he picks by letter, notes outrank recommendations, each pick becomes a rule in Admin.md first. Re-read the canvas (`--extract`) before any republish — he leaves notes on it.
