...
| **ProfileCard** | §9. Large-format media: the picture is the whole card and the words ride a **glass-2 plate** on it. Added 2026-09-11 and reworked the same day — the first version was a full-bleed photo under a gradient scrim and wore none of §3's material. It carries all four cues now: the hairline and the lit top edge as inset shadows *over* the picture, the 118° sweep and the grain at 0.7 strength — while the **words sit directly on the photograph**, white, held legible by a shadow rather than by a panel. That split was argued twice in a day: a `glass-2` plate under the words read as ours but stopped being the thing asked for, so the *card* is the system's and the *type treatment* is the reference's, and neither pretends to be the other. `crop: fill \| cutout` — a cutout stands a background-free figure on the hue's own wash, bottom-aligned. `bloom` renders the media a second time, blurred and scaled beneath the card: the light it casts on the surface, without extracting a colour |
