...
| Board | Pick | The rule |
|---|---|---|
| F1 · a widget cell's head | **A** + note | `CellHead` fills its cell and spreads: the mono label at the top, the title at the bottom — the diagonal of Design-System.md §8.3, restored by the component rather than by the author. **No dot**: the `dot` prop goes (deprecated alias for one minor, ignored). In its place a **`media` slot** — one `Image`, `Blob` or `Pattern`, the company's logo or a picture — drawn at 24px beside the title's first line where the dot was; in a page cell (rows that grow) the head collapses back to a block. The hand-written widgets keep their dots until they are re-poured; the document's do not |
| F2 · the widget text scale | **A** | `Text` gains `size="widget"` — a fourth step, `--t-widget-text` 20/1.35, the scale a widget is read at. One enum value; the inspector's Segmented shows four |
| F3 · the loud cell's word | **A** | `Figure` gains `kind="figure" \| "word"`. As a word: the display face at `--t-widget-word` (48), two lines, `value` capped at 24, the label optional. Same corner, same job |
| F4 · marking sample copy | **A** | `DocumentContext` gains `sampled?: (path: string) => boolean`. Any node whose props resolved a ref into a sampled key renders the *sample copy* tag (`Placeholder`'s) as its first child; the issues list says which ref. A document that inlines sampled text cannot be tagged — sampled copy lives in content and is reached by ref |
| F5 · a list inside a widget | **A** | A `Text` whose markdown is a list, inside a bento cell, renders without markers and keeps its rhythm; F2 gives it the size. One CSS rule in the cell; no new component |
