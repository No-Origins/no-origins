...
Companion documents: **Grid-v2.md** (the field a slot sits on; §8 the composer; D21 the box this document renames),
**Brand.md** (upstream of everything visual), **Admin.md §0.5** (a quest is slots on the grid). Rule numbering is this
document's own, S1 onward.
...
full schema per component, which v1 had and which took weeks (Scene-Schema.md).
...
- **Overflowing children.** A sub-slot that does not fit its parent's field on some breakpoint is dropped silently
  from the render. The composer should flag it, as it flags overlaps.
- ~~Editing sub-slots in place~~ — done by double-click and dimming (§4), 2026-09-21.
...
| The registry | `packages/ui/src/components/registry.tsx` |
| The model: `slot`, `component`, `children` on a layout item | `packages/ui/src/lib/grid-layout.ts` |
| Where slots are placed | `apps/design/src/app/composer/page.tsx` → `design.no-origins.com/composer` |
