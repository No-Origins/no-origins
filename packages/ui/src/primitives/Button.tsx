 ...
 *  - `primary`: ink fill, ground text.  - `secondary`: glass-1 with a hairline.  - `ghost`: text, underline on hover.
...
export function Button({ variant = "primary", size = "md", leading, trailing, href, as, className, children, type, ...rest }: ButtonProps) {
  ...
  return (
    <Tag
      className={cx("noo-btn", `noo-btn--${variant}`, size === "sm" && "noo-btn--sm", variant === "secondary" && "glass glass-1", className)}
      href={href}
