 ...
 *
 * In document mode (§8.6) it is a `Speaker` with the bubble below: a NodeToolbar is positioned against the node's
  ...
  if (flow) {
    return (
      <Speaker
        ...
        label={data.label}
        interactive={Boolean(data.href || data.view)}
        say={speaking ? data.say : undefined}
        tint={data.tint}
        className="noo-canvas__speaker"
      />
    );
  }
