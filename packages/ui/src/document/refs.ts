 ...
 */
export function resolveDeep(value: unknown, content: unknown, path: string, out: Unresolved[]): unknown {
  if (isRef(value)) {
    const got = resolveRef(value.$ref, content);
    if (got === undefined) out.push({ path, ref: value.$ref });
    return got;
  }
  if (Array.isArray(value)) return value.map((v, i) => resolveDeep(v, content, `${path}.${i}`, out));
  if (value && typeof value === "object") {
    const o: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(value as Record<string, unknown>)) o[k] = resolveDeep(v, content, `${path}.${k}`, out);
    return o;
  }
  return value;
}
